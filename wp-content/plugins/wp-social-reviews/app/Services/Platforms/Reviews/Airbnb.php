...
//                'business_type' => $this->businessType,
    ...
    private function tryGraphQLApproach($businessUrl)
    {
        ...
        if(empty($apiKey) || empty($secretKey)) {
            throw new \Exception(__('API Key or Secret Key are missing.', 'wp-social-reviews'));
        }
    ...
    public function formatData($review, $index)
    {
        ...
        return [
            ...
            'reviewer_text' => $reviewData['review_text'],
            'rating'        => $reviewData['rating'],
            'review_time'   => date('Y-m-d H:i:s', strtotime($reviewData['review_date'])),
            'review_approved' => 1,
