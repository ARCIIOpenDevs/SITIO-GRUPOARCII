...
class Hooks {
	...
	public function __construct() {
		...
		if ( igd_fs()->can_use_premium_code__premium_only() ) {
			...
			add_action( 'igd_restore_sharing_interval', [ $this, 'restore_sharing_interval' ] );

			// Automatic creates private folders
			if ( igd_get_settings( 'autoPrivateFolders' ) ) {
	...
	public function restore_sharing_interval() {
		...
		$results = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT file_id, account_id FROM {$table} WHERE type = 'shared' AND created_at BETWEEN DATE_SUB(NOW(), INTERVAL %d HOUR) AND NOW()",
				$interval
			),
			ARRAY_A
		);
		...
		if ( empty( $results ) ) {
			return; // No recent shared files found, nothing to restore
		}
		...
		foreach ( $accounts_files as $account_id => $files ) {
			...
			try {
				...
				error_log( sprintf(
					'Error restoring sharing permissions for account %s: %s',
					$account_id,
					$ex->getMessage()
				) );
	...
	public function cron_schedules__premium_only( $schedules ) {
		$auto_sync = igd_get_settings( 'autoSync' );
		...
		$restore_permissions = igd_get_settings( 'restoreManageSharing' );
		if ( $restore_permissions ) {
			$interval = igd_get_settings( 'restoreManageSharingInterval', 1 );

			if ( 'custom' == $interval ) {
				$custom_interval = igd_get_settings( 'customRestoreManageSharingInterval', 1 );
				$interval        = intval( $custom_interval );
			}

			$interval = $interval * HOUR_IN_SECONDS;

			$schedules['igd_restore_sharing_interval'] = [
				'interval' => $interval,
				'display'  => __( 'Integrate Google Drive Restore Sharing Permissions Interval', 'integrate-google-drive' )
			];
		}
	...
	public function init_cron_job__premium_only() {
		...
		$auto_sync = igd_get_settings( 'autoSync' );
		$sync_hook = 'igd_sync_interval';

		if ( $auto_sync ) {

			$interval = igd_get_settings( 'syncInterval', '3600' );

			$scheduled_interval = igd_get_scheduled_interval( $sync_hook );
			...
			if ( $scheduled_interval != $interval || $interval < 60 ) {
				wp_clear_scheduled_hook( $sync_hook );
			}

			if ( ! wp_next_scheduled( $sync_hook ) ) {
				wp_schedule_event( time(), 'igd_sync_interval', $sync_hook );
			}
		...
		// Restore sharing hook
		$restore_permissions = igd_get_settings( 'restoreManageSharing' );
		$restore_hook        = 'igd_restore_sharing_interval';

		if ( $restore_permissions ) {

			$interval = igd_get_settings( 'restoreManageSharingInterval', 1 );

			$scheduled_interval = igd_get_scheduled_interval( $restore_hook );
			...
			if ( $scheduled_interval != $interval ) {
				wp_clear_scheduled_hook( $restore_hook );
			}

			if ( ! wp_next_scheduled( $restore_hook ) ) {
				wp_schedule_event( time(), 'igd_restore_sharing_interval', $restore_hook );
			}
