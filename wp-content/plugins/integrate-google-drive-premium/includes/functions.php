	...
	if ( ! empty( $files ) ) {
		foreach ( $files as $file ) {
			...
			if ( $direct_image && ( $is_image || $is_audio || $is_video ) ) {
				...
				printf( '<iframe class="igd-embed" src="%1$s" frameborder="0" width="%2$s" height="%3$s" referrerpolicy="no-referrer" allow="autoplay" allowfullscreen="allowfullscreen" %4$s></iframe>', $url, $embed_width, $embed_height, $sandbox_attrs );
			}
		}
	}
