...
class WooCommerce_Uploads {
	...
	public function __construct() {
		...
		// Render Order Received, My Account Page Upload Box
		...
		// Render Upload button to my Order Table
		add_filter( 'woocommerce_my_account_my_orders_actions', [ $this, 'add_orders_column_actions' ], 10, 2 );

		// Render cart page uploader
		add_action( 'woocommerce_after_cart_item_name', [ $this, 'render_cart_page_uploader' ] );

		// Handle Order Metadata Tags
		add_action( 'woocommerce_checkout_order_created', [ $this, 'save_checkout_uploads' ] );
		add_action( 'woocommerce_store_api_checkout_order_processed', [ $this, 'save_checkout_uploads' ] );
		...
		// Add uploader data to cart items in cart/checkout blocks
		add_filter( 'woocommerce_get_item_data', [ $this, 'add_blocks_cart_items_data' ], 99, 2 );
	}

	public function add_blocks_cart_items_data( $data, $cart_item ) {
		...
		$upload_locations = $this->get_woocommerce_upload_locations();
		...
		if ( ! in_array( 'cart', $upload_locations ) && ! in_array( 'checkout', $upload_locations ) ) {
			return $data;
		}

		ob_start();
		$this->render_uploader( $product, null, $this->get_session_files( $product_id ) );
		$uploader_html = ob_get_clean();

		$data[] = [
			'key'   => 'igd_wc_uploader',
			'value' => base64_encode( $uploader_html ),
		];
	...
	public function validate_required_upload_field( $passed, $product_id, $qty ) {
		...
		$module_data      = get_post_meta( $product_id, '_igd_module_data', true );
		$module_data      = ! empty( $module_data ) ? json_decode( base64_decode( $module_data ), true ) : [];
		$upload_min_files = ! empty( $module_data['minFiles'] ) ? $module_data['minFiles'] : 0;
	...
	public function add_product_data_tab( $product_data_tabs ) {
		...
		$product_data_tabs['upload_options'] = [
			'label'  => __( 'Upload to Google Drive', 'integrate-google-drive' ),
	...
	public function add_orders_column_actions( $actions, \WC_Order $order ) {
		...
		if ( $this->requires_order_uploads( $order ) ) {
			...
			foreach ( $order->get_items() as $order_item ) {
				...
				$module_data     = get_post_meta( $product_id, '_igd_module_data', true );
				$module_data     = ! empty( $module_data ) ? json_decode( base64_decode( $module_data ), true ) : [];
				$upload_btn_text = ! empty( $module_data['uploadBtnText'] ) ? $module_data['uploadBtnText'] : __( 'Upload Files', 'integrate-google-drive' );
	...
	public function render_uploader( $product, $order = null, $files = [] ) {
		if ( doing_action( 'woocommerce_email_order_details' ) || ! $this->requires_product_uploads( $product ) ) {
			return;
		}
		...
		$item_id = $order ? $this->get_order_item_id_by_product_id( $order, $product_id ) : null;

		$module_data['isWooCommerceUploader'] = $wc_page;
		$module_data['wcItemId']              = $item_id;
		$module_data['wcOrderId']             = $order ? $order->get_id() : null;
		$module_data['wcProductId']           = $product_id;
		$module_data['uploadedFiles']         = $files;

		if ( $this->is_dokan_active() ) {
			$vendor_id              = get_post_field( 'post_author', $product_id );
			$module_data['account'] = Account::instance( $vendor_id )->get_active_account();
		}

		$button_text = ! empty( $module_data['uploadBtnText'] ) ? $module_data['uploadBtnText'] : __( 'Upload Documents', 'integrate-google-drive' );
        ...
        <div class="igd-wc-upload-wrap">
			...
			// Render Description
			if ( ! empty( $module_data['uploadBoxDescription'] ) ) {
				echo '<p class="igd-wc-upload-description">' . esc_html( $module_data['uploadBoxDescription'] ) . '</p>';
			}
	...
	public function save_checkout_uploads( $order ) {
		...
		foreach ( $items as $item_id => $item ) {
			...
			// Uploader data
			$data = WC()->session->get( 'igd_wc_upload_data_' . $product_id, [] );
			WC()->session->__unset( 'igd_wc_upload_data_' . $product_id );
			...
			$upload_folder_name = ! empty( $data['folderNameTemplate'] ) ? $data['folderNameTemplate'] : 'Order - %wc_order_id% - %wc_product_name% (%user_email%)';
	...
	public function get_upload_folder( $data, $product, $order = null ) {
		$product_id = $product->get_id();
		...
		if ( $order ) {
			...
			$folder = wc_get_order_item_meta( $order_item_id, '_igd_upload_folder' );
		} else {
			$folder = WC()->session->get( 'igd_upload_folder_' . $product_id, [] );
		}

		if ( ! empty( $folder ) ) {
			//check if folder is existing in Google Drive
			try {
				$folder = App::instance( $folder['accountId'] )->get_file_by_id( $folder['id'], true );
			} catch ( \Exception $e ) {
				$folder = $this->create_upload_folder( $data, $product, $order );
			}
	...
	public function create_upload_folder( $data, $product, $order = null ) {
		$product_id = $product->get_id();

		$upload_folder_name = ! empty( $data['folderNameTemplate'] ) ? $data['folderNameTemplate'] : 'Order - %wc_order_id% - %wc_product_name% (%user_email%)';
		$account_id         = ! empty( $data['accountId'] ) ? $data['accountId'] : '';
		$parent_folder_id   = ! empty( $data['folderId'] ) ? $data['folderId'] : '';
		$parent_folder      = [
			'id'        => $parent_folder_id,
	...
	public function requires_product_uploads( $product = null, $order = null ) {
		...
		if ( 'variation' === $product->get_type() && ! empty( wc_get_product( $product->get_parent_id() ) ) ) {
			$product = wc_get_product( $product->get_parent_id() );
		}
	...
	public function add_product_data_tab_content() {
        ...
        <div id="upload_options" class="panel woocommerce_options_panel hidden" style="display:none">
            <div class="options_group">
                <p class="form-field">
                    <label for="_igd_upload"><?php _e( 'Show Upload Box : ', 'integrate-google-drive' ); ?></label>

					<?php echo wc_help_tip( __( 'Show upload box to allow customers to upload files for this product.', 'integrate-google-drive' ) ); ?>
                ...
                <div class="show_if_igd_upload upload-box-settings">
                    <h4><?php _e( 'Google Drive Upload Box Settings', 'integrate-google-drive' ); ?></h4>
	...
	public function save_product_meta( $post_id ) {
		...
		$igd_module = ! empty( $_POST['_igd_module_data'] ) ? sanitize_text_field( $_POST['_igd_module_data'] ) : '';

		update_post_meta( $post_id, '_uploadable', $uploadable );
		update_post_meta( $post_id, '_igd_upload', $igd_upload );
		update_post_meta( $post_id, '_igd_module_data', $igd_module );
	}
