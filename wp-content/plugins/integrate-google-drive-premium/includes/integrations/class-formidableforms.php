...
class FormidableForms {
	...
	public function add_hooks() {
		...
		add_action( 'frm_field_options_form_top', [ $this, 'field_options_form' ], 5, 3 );
		...
		add_action( 'frm_enqueue_builder_scripts', [ $this, 'enqueue_scripts' ] );
	...
	public function may_create_entry_folder( $entry_id, $form_id ) {
		...
		if ( ! empty( $igd_fields ) ) {
			...
			foreach ( $igd_fields as $field ) {
				...
				$shortcode_data = Shortcode::get_shortcode( $field->field_options['module_id'] )['config'] ?? [];
				...
				if ( $private_folders && ! empty( $entry->user_id ) ) {
					$folders = get_user_meta( $entry->user_id, 'igd_folders', true );
	...
	public function field_options_form( $field, $display, $values ) {
		...
		if ( ! isset( $field['module_id'] ) ) {
			$field['module_id'] = $this->default_value;
		}
		...
		if ( ! empty( $shortcodes ) ) {
			foreach ( $shortcodes as $shortcode ) {
                $options .= sprintf(
                    ...
                    selected( $field['module_id'], $shortcode['id'], false ),
        ...
        <div class="igd-configure-field">
            <label for="module_id_<?php echo esc_attr( $field['id'] ); ?>"
                   class="frm_help"
                   data-original-title="<?php esc_attr_e( 'Select an existing Google Drive module or create a new one.', 'integrate-google-drive' ); ?>"
            ><?php esc_html_e( 'Select Module', 'integrate-google-drive' ); ?></label>

            <select name="field_options[module_id_<?php echo esc_attr( $field['id'] ); ?>]"
                    id="module_id_<?php echo esc_attr( $field['id'] ); ?>"
                    class="frm_long_input igd-uploader-data">
                <?php echo $options; ?>
            </select>
	...
	public function frontend_field( $field, $field_name, $atts ) {
		...
		$module_id      = $field['module_id'] ?? $this->default_value;
	...
	public function validation( $errors, $posted_field, $posted_value, $args ) {
		...
		$shortcode_data = json_decode( $posted_field->field_options['module_id'], true );
		$min_files      = isset( $shortcode_data['minFiles'] ) ? $shortcode_data['minFiles'] : 0;
