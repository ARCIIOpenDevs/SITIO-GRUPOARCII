...
class Media_Library {
	...
	public function __construct() {
		...
		// Insert attachment after upload if parent is in media folders
		$media_folders = igd_get_settings( 'mediaLibraryFolders', [] );

		if ( ! empty( $media_folders ) ) {
			add_action( 'igd_upload_post_process', function ( $file ) use ( $media_folders ) {

				$parent_folder_id = $file['parents'][0] ?? '';

				if ( empty( $parent_folder_id ) || empty( $file['id'] ) ) {
					return;
				}
				...
				$parent_folder_ids = wp_list_pluck( $parent_folders, 'id' );
				$media_folder_ids  = wp_list_pluck( $media_folders, 'id' );

				// Check for direct parent match
				if ( in_array( $parent_folder_id, $media_folder_ids, true ) && in_array( $parent_folder_id, $parent_folder_ids, true ) ) {
					$this->insert_attachment( $file );
				}
		...
		// get attachments linked Google Drive files
		add_action( 'wp_ajax_igd_get_attachments_linked_files', [ $this, 'get_attachments_linked_files' ] );

	}

	public function get_attachments_linked_files() {
		...
		$attachment_ids = ! empty( $_REQUEST['attachment_ids'] ) ? array_map( 'intval', $_REQUEST['attachment_ids'] ) : [];

		if ( empty( $attachment_ids ) ) {
			wp_send_json_error( __( 'No attachment IDs provided', 'integrate-google-drive' ) );
		}
		...
		if ( empty( $linked_files ) ) {
			wp_send_json_error( __( 'No linked files found for the provided attachments', 'integrate-google-drive' ) );
		}

		wp_send_json_success( $linked_files );
		...
		if ( is_array( $image ) && ! empty( $file_id ) ) {
			...
			if ( $size === 'full' ) {
				...
				$thumb_url = igd_get_thumbnail_url( $file, 'custom', [
					'width'  => $has_thumbnail ? $image[1] : 64,
					'height' => $has_thumbnail ? $image[2] : 64,
				] );
		...
		if ( ! empty( $file_id ) ) {
			...
			if ( empty( $replace_id ) && ! empty( $meta['sizes'] ) && is_array( $meta['sizes'] ) ) {
				...
				foreach ( $meta['sizes'] as $size => $size_info ) {
					...
					$thumb_url = igd_get_thumbnail_url( $file, 'custom', [
						'width'  => $has_thumbnail ? $size_info['width'] : 64,
						'height' => $has_thumbnail ? $size_info['height'] : 64,
					] );
	...
	public function render_metadata( $attachment_id, $file ) {
		...
		if ( ! empty( $medium_size_w ) && ! empty( $medium_size_h ) ) {
			...
			$thumbnail = igd_get_thumbnail_url( $file, 'custom', [
				'width'  => $has_thumbnail ? $medium_size_w : 64,
				'height' => $has_thumbnail ? $medium_size_h : 64,
			] );
		...
		if ( ! empty( $large_size_w ) && ! empty( $large_size_h ) ) {
			...
			$thumbnail = igd_get_thumbnail_url( $file, 'custom', [
				'width'  => $has_thumbnail ? $large_size_w : 64,
				'height' => $has_thumbnail ? $large_size_h : 64,
			] );
