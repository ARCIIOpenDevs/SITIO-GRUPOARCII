...
class GravityForms extends \GFAddOn {
	...
	public function may_create_entry_folder( $entry, $form ) {
		...
		if ( ! empty( $igd_fields ) ) {
			foreach ( $igd_fields as $field ) {
				...
				$upload_folder = ! empty( $shortcode_data['folders'] ) ? reset( $shortcode_data['folders'] ) : [
					'id'        => 'root',
					'accountId' => '',
				];
				...
				if ( $private_folders && ! empty( $entry['created_by'] ) ) {
					$folders = get_user_meta( $entry['created_by'], 'igd_folders', true );
	...
	private function handle_form_field_tags( $name_template, $form_fields, $entry ) {
		...
		if ( ! empty( $field_ids ) ) {
			foreach ( $form_fields as $tagField ) {
				...
				if ( empty( $entry[ $tagField->id ] ) ) {
					// implode all items of the array that key starts with $tagField->id. (with a dot)
					$field_value = '';

					foreach ( $entry as $key => $value ) {
						if ( ! empty( $value ) && strpos( $key, $tagField->id . '.' ) === 0 ) {
							$field_value .= $value . ' ';
						}
					}
	...
	public function custom_field_settings( $position, $form_id ) {
		if ( 1430 == $position ) {
            ...
            <li class="igd_settings field_setting">
                <label for="field_module_id_<?php echo esc_attr( $this->_slug ); ?>"><?php esc_html_e( 'Select Module', 'integrate-google-drive' ); ?><?php echo gform_tooltip( 'form_field_' . $this->_slug ); ?></label>

                <select
                        id="field_module_id_<?php echo esc_attr( $this->_slug ); ?>"
                        class="igd-uploader-data" onchange="SetFieldProperty('module_id', this.value)">
					<?php echo $options; ?>
                </select>

                <button id="igd-form-uploader-config-gravityforms"
                        type="button"
                        class="igd-form-uploader-trigger igd-btn btn-primary"
                >
	...
	public function add_tooltip( $tooltips ) {
		$tooltips[ 'form_field_' . $this->_slug ] = esc_html__( 'Enter the existing module id or create a new module.', 'integrate-google-drive' );
...
class GravityForms_Field extends \GF_Field {
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		...
		$shortcode_data = Shortcode::get_shortcode( $this->module_id )['config'] ?? [];;
		...
		return sprintf( '<div class="gform-theme__disable">%s</div>', $input );
	}
	...
	public function validate( $value, $form ) {
		...
		if ( ! empty( $shortcode_data['minFiles'] ) && ( empty( $attached_files ) || count( $attached_files ) < $shortcode_data['minFiles'] ) ) {
			$this->failed_validation = true;

			if ( ! empty( $this->errorMessage ) ) {
				$this->validation_message = $this->errorMessage;
			} else {
				$this->validation_message = sprintf( esc_html__( 'Please upload at least %d files.', 'integrate-google-drive' ), $shortcode_data['minFiles'] );
			}
		}
	...
	public function get_value_merge_tag( $value, $input_id, $entry, $form, $modifier, $raw_value, $url_encode, $esc_html, $format, $nl2br ) {
		return $this->render_uploaded_files( html_entity_decode( $value ), ( 'html' === $format ) );
	}
	...
	public function get_value_entry_list( $value, $entry, $field_id, $columns, $form ) {
		if ( ! empty( $value ) ) {
			return $this->render_uploaded_files( html_entity_decode( $value ) );
		}
	}
	...
	public function get_value_export( $entry, $input_id = '', $use_text = false, $is_csv = false ) {
		...
		return $this->render_uploaded_files( html_entity_decode( $value ), false );
	}

	public function render_uploaded_files( $data, $as_html = true ) {
