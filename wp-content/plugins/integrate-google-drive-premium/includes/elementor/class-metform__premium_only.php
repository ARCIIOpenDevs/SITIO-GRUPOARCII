...
class Metform extends Widget_Base {
	...
	public function register_controls() {
		...
		$this->add_control( 'edit_field_metform', [
			...
			'label'       => '',
			'text'        => '<i class="eicon eicon-settings"></i>' . __( 'Configure', 'integrate-google-drive' ),
			'event'       => 'igd:editor:edit_module',
			'description' => __( 'Select a module or create a new one.', 'integrate-google-drive' ),
		] );
	...
	public function render() {
		...
		$module_data = Shortcode::get_shortcode( $module_id )['config'] ?? [];

		$data = wp_parse_args( $module_data, $default_data );

		$configData = [
			'message'  => isset( $mf_input_validation_warning_message ) ? ! empty( $mf_input_validation_warning_message ) ? $mf_input_validation_warning_message : esc_html__( 'This field is required.', 'integrate-google-drive' ) : esc_html__( 'This field is required.', 'integrate-google-drive' ),
