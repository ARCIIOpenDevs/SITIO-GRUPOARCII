...
class Shortcode {
	...
	public function render_shortcode( $atts = [], $data = null ) {
		...
		// Check if the module required password protection or email
		if ( igd_fs()->can_use_premium_code__premium_only() ) {

			// Check if the module is password protected
			if ( Restrictions::instance()->should_password_protected( $this->data ) ) {
				return $this->get_password_protected_screen();
			}

			// Check if the module requires email
			if ( 'review' == $this->type || ( 'gallery' === $this->type && ! empty( $this->data['photoProof'] ) ) ) {

				if ( ! is_user_logged_in() && empty( $_COOKIE['igd_user_email'] ) ) {
					return $this->get_email_required_screen();
				}

				if ( is_user_logged_in() ) {
					$user_email = wp_get_current_user()->user_email;
				} else {
					$user_email = sanitize_email( $_COOKIE['igd_user_email'] );
				}

				// Fetch the selection based on the selection ID
				if ( ! empty( $_GET['selection_id'] ) ) {
					$selection = Proof_Selections::instance()->get_selections(
						[
							'id'           => intval( $_GET['selection_id'] ),
							'shortcode_id' => $this->data['id'],
						]
					);
				} else { // Fetch the selection based on the user email
					$selection = Proof_Selections::instance()->get_selections( [
						'email'        => $user_email,
						'shortcode_id' => $this->data['id'],
					] );
	...
	public function enqueue_scripts() {
		...
		if ( igd_fs()->can_use_premium_code__premium_only() ) {
			...
			// Dequeue script to load igd-frontend at the end
			wp_dequeue_script( 'igd-frontend' );
			...
			} elseif ( in_array( $this->type, [ 'browser', 'gallery', 'review' ] ) ) {

				$can_delete   = ! empty( $this->data['canDelete'] );
				$can_move     = ! empty( $this->data['moveCopy'] );
				$can_download = ! empty( $this->data['download'] ) && ! empty( $this->data['zipDownload'] );

				// Enqueue Drag-Select
				if ( 'review' == $this->type || $can_delete || $can_move || $can_download ) {
