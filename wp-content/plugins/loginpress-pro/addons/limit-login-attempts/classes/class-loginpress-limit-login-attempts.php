		...
		public function loginpress_login_attempts_log_table_size() {
			...
			if ($row_count < $row_threshold) {
				...
				return sprintf(	// translators: Notice to optimize performance
					__('%1$s Your logs table contains approximately %3$s%2$s rows %4$s. Consider clearing them to optimize database performance.%5$s', 'loginpress-pro'), '<span class="loginpress_table_size_notify">',
					esc_html(number_format($row_count)), '<strong>', '</strong>','</span>'
				);
			}
		}		
